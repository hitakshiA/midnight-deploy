...
import { NetworkId as ZswapNetworkId, nativeToken, } from "@midnight-ntwrk/zswap";
import { setNetworkId, NetworkId as GlobalNetworkId, } from "@midnight-ntwrk/midnight-js-network-id";
import { configureProviders, waitForFunds, } from "./providers.js";
...
// Deploy function
// --------------------
export async function deploy(configPath, quickDeploy, providedSeed) {
    ...
    try {
        ...
        const config = jiti(configFilePath);
        let deployerHexSeed;
        if (quickDeploy) {
            deployerHexSeed = bytesToHex(randomBytes(32));
            spinner.text = chalk.yellow("Quick deploy: Generated random hex seed for deployer.");
        }
        else if (providedSeed) {
            deployerHexSeed = providedSeed;
            spinner.text = chalk.yellow("Using hex seed provided via --seed option.");
        }
            ...
            throw new Error("Deployer hex seed not found. Use --quick-deploy, --seed, or set DEPLOYER_HEX_SEED in .env");
        }
        if (!deployerHexSeed || !/^[0-9a-fA-F]{64}$/.test(deployerHexSeed)) {
            throw new Error("Invalid deployer hex seed format. Must be 64 hexadecimal characters.");
        }
        ...
        for (const contractConfig of config.contracts) {
            ...
            const sourcePath = path.resolve(path.dirname(configFilePath), contractConfig.path);
            const managedPath = path.join(path.dirname(sourcePath), "managed", contractConfig.name);
            execSync(`/home/akash/midnight/bin/compactc ${sourcePath} ${managedPath}`, { stdio: "pipe" });
            spinner.succeed(`Compiled ${contractName}`);
            // ✅ require CJS artifact
            const { Contract } = customRequire(path.join(managedPath, "contract/index.cjs"));
            // --------------------
            // Wallet
            // --------------------
            spinner.start(`Building wallet for deployment...`);
            const walletSeed = fullSeed.slice(0, 32);
            const wallet = await WalletBuilder.build("https://indexer.testnet-02.midnight.network/api/v1/graphql", "wss://indexer.testnet-02.midnight.network/api/v1/graphql/ws", "http://localhost:6300", "https://rpc.testnet-02.midnight.network", bytesToHex(walletSeed), ZswapNetworkId.TestNet, "info");
            wallet.start();
            ...
            spinner.succeed(`Wallet synced with sufficient balance: ${walletState.balances[nativeToken()] || 0n} tDUST`);
            // --------------------
            // Deploy
            // --------------------
            ...
            spinner.succeed(`Deployed ${contractName} at ${chalk.bold(contractAddress)}`);
            // --------------------
            // Init if needed
            // --------------------
            if (contractConfig.init) {
                ...
                spinner.succeed(`Initialized ${contractName}! (Tx ID: ${initTx.public.txId})`);
            }
        ...
        await fs.writeFile(path.resolve(process.cwd(), "deployments.json"), deploymentsJson);
